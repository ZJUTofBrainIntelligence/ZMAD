...
1DCNN
'''
class One_DCNN_AccData(nn.Module):
    def __init__(self):
        super(One_DCNN_AccData, self).__init__()
        ...
        self.fc1 = nn.Linear(384, 128)  # 修改输入维度为 512
...
class LSTMModel_Acc(nn.Module):
    def __init__(self, input_dim=3, hidden_dim=128, output_dim=37, dropout_rate=0.):
        super(LSTMModel_Acc, self).__init__()
        # 定义第一层 LSTM
...
class BiLSTMModel_Acc(nn.Module):
    def __init__(self, input_dim=3, hidden_dim=512, output_dim=37, dropout_rate=0.5):
        super(BiLSTMModel_Acc, self).__init__()
        ...
        for name, param in self.named_parameters():
            if 'bilstm' in name:
                if 'weight_ih' in name:
                    ...
                    # 设置LSTM的偏置为一个合适的常数，可能有助于忘记门的更好学习
                    n = param.size(0)
                    start, end = n//4, n//2
